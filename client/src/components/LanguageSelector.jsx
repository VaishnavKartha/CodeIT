    ...
    <div className='mt-2 mb-1 border w-fit mx-auto'>
      <div className='p-1 relative flex items-center'>
        <div className='flex items-center gap-2'>
            <span className='text-md'>{activeLanguage}</span>
            <span className='text-[12px] text-zinc-500'>{activeLanguage?.version}</span> 
        </div>
        
            <span className='cursor-pointer' onClick={()=>setIsOpen(!isOpen)}> {!isOpen?<ChevronDown size={"20px"}/>:<ChevronUp size={"20px"}/> }</span>
            ...
            {isOpen && <div className='z-2 absolute w-full top-full bg-[#181818] transform translate-y-px left-0 transition-all duration-300 ease-in-out'> 
                    {LANGUAGES.length > 0 && LANGUAGES.map((language,idx)=>{
                        if(language.lang === activeLanguage) return
                return <div className='flex items-center gap-2 cursor-pointer hover:bg-zinc-800 px-1 py-2' key={idx} onClick={()=>setActiveLanguage(language)}>
